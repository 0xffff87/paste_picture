...
- 原生 Win32 反馈窗口，始终置顶，**弹出时不抢占焦点**
- **图片粘贴** — 可以直接粘贴截图或剪贴板中的图片到反馈窗口；图片保存为 PNG 并通过 MCP 以 base64 编码发送给 AI
- 支持拖拽文件到窗口或粘贴已复制的文件，插入路径到输入框
- **窗口复用** — 当新的反馈请求到达时，如果窗口已打开，会复用现有窗口（更新摘要、任务栏闪烁提示），不会弹出重复窗口
...
3. 否则，服务端检查是否已有 GUI 窗口在运行：
   - **如果有** — 通过 `WM_COPYDATA` 发送新摘要给现有窗口，窗口就地更新（任务栏闪烁提示用户）。
   - **如果没有** — 启动新的 `feedback-gui.exe`，使用 `SW_SHOWNOACTIVATE` 显示，置顶但不抢占键盘焦点。
4. 服务端等待以下事件之一：
   - **用户提交** — GUI 将文本反馈写入临时文件，如果有粘贴的截图则将图片路径写入 `.images` 附属文件。服务端读取后将图片 base64 编码，连同文本一起作为 MCP content 返回给 AI。非空反馈会重置自动回复的循环索引。
...
```
Cursor  ←— stdio JSON-RPC —→  feedback-server.exe  ←— 命令行参数 / 临时文件 / .images —→  feedback-gui.exe
                                                    ←— Win32 消息 (WM_COPYDATA) ————————→
```

两个进程：MCP 服务端通过 stdin/stdout 与 Cursor 通信（JSON-RPC 2.0，每行一个 JSON）。GUI 是独立的 Win32 进程——必须分离因为 Cursor 占用了 stdin/stdout。两者之间的通信：命令行参数（启动时）、`WM_COPYDATA`（更新摘要以复用窗口）、Windows 消息（超时/取消通知）、临时文件（返回文本反馈）、`.images` 附属文件（返回粘贴的图片路径）。
...
基于 [junanchn](https://github.com/junanchn) 的 [Interactive Feedback MCP](https://github.com/junanchn/interactive-feedback-mcp) 原始项目开发。
